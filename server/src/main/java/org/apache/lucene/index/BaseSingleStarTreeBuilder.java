...
public abstract class BaseSingleStarTreeBuilder implements SingleTreeBuilder {
    ...
    protected BaseSingleStarTreeBuilder(
        ...
        numMetrics = metricStatFieldPairs.size();
     ...
     * @throws IOException throws an exception if we are unable to create sorted doc values indices
     */
    // TODO: To be enabled once we have star tree codec
    private void createSortedDocValuesIndices(DocValuesConsumer docValuesConsumer) throws IOException {
        ...
        for (int i = 0; i < dimensionReaders.length; i++) {
            ...
            final FieldInfo dimensionFieldInfo = new FieldInfo(
            ...
            dimensionFieldInfoList[i] = dimensionFieldInfo;
            StarTreeDocValuesWriter starTreeDimensionDocValuesWriter = new StarTreeDocValuesWriter(
                originalDimensionFieldInfo.getDocValuesType(),
                getDocValuesWriter(originalDimensionFieldInfo.getDocValuesType(), dimensionFieldInfo, Counter.newCounter())
            );
        ...
        for (int i = 0; i < metricReaders.length; i++) {
            ...
            FieldInfo metricFieldInfo = new FieldInfo(
                metrics[i] + "_metric",
                fieldNum,
                false,
            ...
            metricFieldInfoList[i] = metricFieldInfo;
            StarTreeDocValuesWriter starTreeMetricDocValuesWriter = new StarTreeDocValuesWriter(
                originalMetricFieldInfo.getDocValuesType(),
                getDocValuesWriter(originalMetricFieldInfo.getDocValuesType(), metricFieldInfo, Counter.newCounter())
            );
        ...
        for (int docId = 0; docId < numDocs; docId++) {
            ...
            for (int i = 0; i < starTreeDocument.dimensions.length; i++) {
                long dimensionValue = starTreeDocument.dimensions[i];
                StarTreeDocValuesWriter starTreeDocValuesWriter = dimensionWriters.get(i);
                switch (starTreeDocValuesWriter.getDocValuesType()) {
                    case SORTED_SET:
                        if (dimensionValue == -1) continue;
                        ((SortedSetDocValuesWriter) starTreeDocValuesWriter.getDocValuesWriter()).addValue(
                            docId,
                            getSortedSetDocValueBytes(dimensionValue, i, ordinalToSortedSetDocValueMap)
                        );
                        break;
                    case SORTED_NUMERIC:
                        ((SortedNumericDocValuesWriter) starTreeDocValuesWriter.getDocValuesWriter()).addValue(docId, dimensionValue);
                        break;
                    default:
    ...
    /**
     * Fetches the bytes ref for ordinal number
     * @param dimensionValue ordinal number of the dimension
     * @param counter fetches the particular dimension
     * @param ordinalToSortedSetDocValueMap maintains the map of dimension name to ordinal number and bytes ref
     * @return returns the bytes ref corresponding to the ordinal number of the dimension
     * @throws IOException throws an exception if we are unable to fetch bytes ref
     */
    private BytesRef getSortedSetDocValueBytes(
        long dimensionValue,
        int counter,
        Map<String, Map<Long, BytesRef>> ordinalToSortedSetDocValueMap
    ) throws IOException {
        ...
        BytesRef bytes;
        if (ordinalToSortedSetDocValueMap.get(dimensionName).containsKey(dimensionValue)) {
            bytes = ordinalToSortedSetDocValueMap.get(dimensionName).get(dimensionValue);
        } else {
            bytes = ((SortedSetDocValues) dimensionReaders[counter]).lookupOrd(dimensionValue);
            ordinalToSortedSetDocValueMap.get(dimensionName).put(dimensionValue, BytesRef.deepCopyOf(bytes));
        }
        return bytes;
    }
     ...
     * @param docValuesConsumer document values consumer
     * @param docValuesWriters document values writers
     * @param fieldInfoList field info list
     * @param readers document set iterators
     * @throws IOException throws an exception if we are unable to get star-tree doc value producers
     */
     ...
     * @param docValuesType doc values type
     * @param fi field info
     * @param counter iteration
     * @return doc values writer based on doc values type
     */
